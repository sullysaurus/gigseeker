...
export async function POST(request: Request) {
  try {
    const { template, venueName, venueDetails, venueId, pipelineVenueId } = await request.json()
    ...
    const creditCost = getAICreditCost('email_personalization')
    ...
    const fullPrompt = `You are an expert music industry email writer. Personalize this email template for a specific venue.
...
Venue Details:
- Name: ${venueName}
${venueDetails ? `- Additional info: ${venueDetails}` : ''}

Instructions:
- Replace placeholders with venue-specific details
- Make the email feel personalized and researched
...
Return JSON only.`
    ...
    const responseText = await generateAIResponse({
      actionType: 'email_personalization',
      prompt: fullPrompt,
      temperature: 0.7,
      maxTokens: 1024,
    })
    ...
    try {
      await supabase.rpc('record_ai_action', {
        p_user_id: user.id,
        p_action_type: 'email_personalization',

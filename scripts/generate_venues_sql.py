#!/usr/bin/env python3
"""
Generate SQL script to insert cleaned venues into Supabase
"""
...
def generate_sql(csv_file, output_file):
    ...
    sql_lines = []
    sql_lines.append("-- Generated Venues Insert Script")
    sql_lines.append("-- Cleaned and deduplicated venue data")
    sql_lines.append(f"-- Total venues: {len(venues)}")
    sql_lines.append("-- Generated: " + str(__import__('datetime').datetime.now()))
    sql_lines.append("")
    sql_lines.append("BEGIN;")
    sql_lines.append("")
    sql_lines.append("-- Delete existing venues (optional - comment out if you want to keep existing)")
    sql_lines.append("-- DELETE FROM public.venues;")
    sql_lines.append("")
    ...
    for city in sorted(by_city.keys()):
        sql_lines.append(f"-- {city}, NC venues ({len(by_city[city])} venues)")
        ...
        for venue in by_city[city]:
            ...
            if venue.get('id'):
                fields.append('id')
                values.append(escape_sql_string(venue['id']))
            ...
            if venue.get('booking_notes') and venue['booking_notes'] != 'null':
                fields.append('booking_notes')
                values.append(escape_sql_string(venue['booking_notes']))
...
if __name__ == '__main__':
    generate_sql(
        'supabase/seeds/venues_cleaned.csv',
        'supabase/seeds/insert_venues_cleaned.sql'
    )
